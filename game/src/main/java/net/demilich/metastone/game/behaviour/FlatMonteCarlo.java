...
public class FlatMonteCarlo extends Behaviour {
	...
	public GameAction requestAction(GameContext context, Player player, List<GameAction> validActions) {
		...
		System.out.println("Se selecciona: "+ bestAction.getActionType() + " con score: "+ actionScores.get(bestAction));
		return bestAction;
	}

	private double simulate(GameContext context, int playerId, GameAction action) { //esto esta simulando solamente y retornando valores
		GameContext simulation = context.clone();
		System.out.println("Se simula: "+ action.getActionType());
		simulation.getLogic().performGameAction(simulation.getActivePlayer().getId(), action);
		if (simulation.gameDecided()) { //comprueba si es jugada para ganar partida
			return simulation.getWinningPlayerId() == playerId ? 1 : 0;
		}
		double score = 0;
		for (int i = 0; i < iterations; i++) {
			score += playRandomUntilEnd(simulation.clone(), playerId);// esta mierda esta seleccionando aleatoriamente cosas
		}
